...
router.post('/api/settings', requireAdminAuth, (req, res) => {
  const { ROUND_DELAY_MS, REQUEST_TIMEOUT_MS, STREAM_READ_TIMEOUT_MS, NVIDIA_API_URL, PORT, MAX_ROUNDS_PER_MODEL, MAX_EMPTY_RESPONSE_RETRIES, TEST_TIMEOUT_MS, MODEL_FAILURE_COOLDOWN_MS, KEY_CONCURRENCY_DELAY_MS, ENABLE_CONTENT_VALIDATION, PRICE_PER_MILLION_PROMPT_TOKENS, PRICE_PER_MILLION_COMPLETION_TOKENS, REF_PRICE_PER_MILLION_PROMPT_TOKENS, REF_PRICE_PER_MILLION_COMPLETION_TOKENS, CURRENCY_SYMBOL } = req.body;
  const current = settings.get();
  ...
  addLog('info', `已更新參數設定：每輪等待 ${(updated.ROUND_DELAY_MS / 1000)}秒, 請求逾時 ${(updated.REQUEST_TIMEOUT_MS / 1000)}秒, 串流逾時 ${(updated.STREAM_READ_TIMEOUT_MS / 1000)}秒, 測試逾時 ${(updated.TEST_TIMEOUT_MS / 1000)}秒, 模型失敗冷卻 ${(updated.MODEL_FAILURE_COOLDOWN_MS / 1000)}秒, 金鑰防併發等待 ${(updated.KEY_CONCURRENCY_DELAY_MS / 1000)}秒, URL: ${updated.NVIDIA_API_URL}, PORT: ${updated.PORT}, 最大重試: ${updated.MAX_ROUNDS_PER_MODEL}輪, 空回傳重試: ${updated.MAX_EMPTY_RESPONSE_RETRIES}次, 內容校驗: ${updated.ENABLE_CONTENT_VALIDATION ? '啟用' : '停用'}, 實際單價: Prompt ${updated.PRICE_PER_MILLION_PROMPT_TOKENS} / Completion ${updated.PRICE_PER_MILLION_COMPLETION_TOKENS} (${updated.CURRENCY_SYMBOL}/百萬tokens), 參考單價: Prompt ${updated.REF_PRICE_PER_MILLION_PROMPT_TOKENS} / Completion ${updated.REF_PRICE_PER_MILLION_COMPLETION_TOKENS}`);
